...
text_model = genai.GenerativeModel(model_name="gemini-pro", generation_config=text_generation_config,
...
async def on_ready():
    ...
    await channel.send("Ready!\nReply to this bot to prompt.")
    await channel.send(f"Text model temperature set to {text_generation_config.temperature}")
    await channel.send(f"Vision model temperature set to {image_generation_config.temperature}")
    await channel.send(f"Max history per person set to {MAX_HISTORY} messages")
